...
class CareCoordinatorApp {
    ...
    async sendChatMessage(message) {
        ...
        try {
            ...
            if (data.form_updates) {
                this.logActivity(`Form updates detected: ${Object.keys(data.form_updates).join(', ')}`);
            }
    ...
    updateAppointmentForm(formUpdates) {
        this.logActivity(`Updating appointment form with ${Object.keys(formUpdates).length} fields`);
        
        const updatedFields = this.setFormFieldValues(formUpdates);
        this.handleFormDependencies(formUpdates, updatedFields);
        this.showFormUpdateResults(updatedFields);
    }

    setFormFieldValues(formUpdates) {
        const updatedFields = [];
        ...
        Object.entries(formUpdates).forEach(([field, value]) => {
            ...
            if (!element) {
                this.logActivity(`ERROR: Form field not found: "${field}"`);
                return;
            }

            // Set field value with appropriate handling
            this.setFieldValue(element, field, value);
            
            // Enable disabled fields as needed
            if (element.disabled && ['appointment-date', 'appointment-time'].includes(field)) {
                element.disabled = false;
            }
            
            // Add visual feedback
            this.addFieldUpdateAnimation(element);
    ...
    setFieldValue(element, field, value) {
        if (element.tagName === 'SELECT' && ['appointment-location', 'doctor'].includes(field)) {
            const matchedOption = this.findMatchingOption(element, value);
            element.value = matchedOption ? matchedOption.value : value;
        } else {
            element.value = value;
        }
    }

    addFieldUpdateAnimation(element) {
    ...
    handleFormDependencies(formUpdates, updatedFields) {
        const needsDoctorHandler = formUpdates.doctor && !formUpdates['appointment-location'];
        const needsDateHandler = formUpdates['appointment-date'];
        
        if (needsDoctorHandler) {
            this.handleDoctorSelection();
        } else if (formUpdates.doctor) {
            // Set up date restrictions without triggering full handler
            this.setupDateRestrictions(formUpdates.doctor);
            document.getElementById('appointment-date').disabled = false;
        }
        
        if (needsDateHandler) {
            this.handleDateSelectionWithTimePreservation(formUpdates['appointment-time']);
        }
    }

    handleDateSelectionWithTimePreservation(aiSetTime) {
        this.handleDateSelection();
    ...
    showFormUpdateResults(updatedFields) {
    ...
    findMatchingOption(selectElement, searchValue) {
        ...
        const matchers = [
            // Exact matches
            option => option.value === searchValue || option.textContent === searchValue,
            option => option.value.toLowerCase() === searchLower || option.textContent.toLowerCase() === searchLower,
            // Substring matches
            option => option.value.toLowerCase().includes(searchLower) || option.textContent.toLowerCase().includes(searchLower),
            option => searchLower.includes(option.value.toLowerCase()) || searchLower.includes(option.textContent.toLowerCase())
        ];
